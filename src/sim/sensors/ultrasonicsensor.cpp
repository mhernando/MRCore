...
namespace mr
{
	...
	UltrasonicSensor::UltrasonicSensor()
	{
		...
		//el cristal se pinta pero no se usa en la detecci�n
	...
	void UltrasonicSensor::setSensorProperties(double _conusangle, double _maxrange, double _minrange, double _gaussian_error)
	{
		...
		//CONO con el eje de rotaci�n en x una recta cada 5 � de latitud y cada 5� de altitud son unos 15� de apertura
		vectorBeam.clear();
		...
		for (int i = 0; i < absoluteVectorBeam.size(); i++) {
			...
			for (int j = 0; j < num; j++)
			{
				...
				if (list[j]->rayIntersection(pos, absoluteVectorBeam[i], dist, &n)) {
					...
					//de momento tomo el valor m�nimo.. pero aqu� se podr�a hacer m�s con ruido y tal
					if (dist < measure) {
