...
bool  RobotSim::forwardKinematics(const vector<double> & _q, Transformation3D& t, unsigned char &conf)//conf puntero salida
{
	int i;
	...
	if(!getConfigurationOf(_q,conf))return false; //obtains the robot config for that q values
